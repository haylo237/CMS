        ...
        <div class="bg-white rounded-xl border shadow-sm p-5 hover:shadow-md transition">
            <div class="flex items-start justify-between mb-3">
                ...
                <div class="flex gap-2 text-gray-400">
                    @can('manage-departments')
                        <a href="{{ route('departments.edit', $dept) }}" class="hover:text-amber-500 transition"><i class="fa-solid fa-pen-to-square"></i></a>
                        <form method="POST" action="{{ route('departments.destroy', $dept) }}" onsubmit="return confirm('Delete department?')">
                            @csrf @method('DELETE')
            ...
            <div class="flex items-center justify-between text-sm">
                ...
                <a href="{{ route('departments.show', $dept) }}" class="text-indigo-600 hover:underline">View →</a>
            </div>
        </div>
    @empty
