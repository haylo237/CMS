            ...
            <div>
                ...
                <select id="countryCodeSelect" name="country_code_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    ...
                    @foreach($countryCodes as $countryCode)
                        <option value="{{ $countryCode->id }}" data-iso="{{ $countryCode->iso_code }}" @selected((string) old('country_code_id', $branch->country_code_id) === (string) $countryCode->id)>
                ...
                <div>
                    ...
                    <select id="regionSelect" name="region_id" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        @foreach($regions as $region)
                            <option value="{{ $region->id }}" @selected((string) old('region_id', $branch->region_id) === (string) $region->id)>
...
populateDivisions(@json(old('division_id', $branch->division_id)));
populateSubdivisions(@json(old('subdivision_id', $branch->subdivision_id)));
</script>
@endsection
