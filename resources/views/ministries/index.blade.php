    ...
    @forelse($ministries as $ministry)
        <div class="bg-white rounded-xl border shadow-sm p-5 hover:shadow-md transition">
            <div class="flex items-start justify-between mb-3">
                ...
                @can('manage-ministries')
                    <div class="flex gap-2 text-gray-400">
                        <a href="{{ route('ministries.edit', $ministry) }}" class="hover:text-amber-500 transition"><i class="fa-solid fa-pen-to-square"></i></a>
                        <form method="POST" action="{{ route('ministries.destroy', $ministry) }}" onsubmit="return confirm('Delete ministry?')">
                            @csrf @method('DELETE')
            ...
            <div class="flex items-center justify-between text-sm">
                ...
                <a href="{{ route('ministries.show', $ministry) }}" class="text-indigo-600 hover:underline">View →</a>
            </div>
        </div>
    @empty
